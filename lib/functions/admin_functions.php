...
function login($user,$pass)
{
	...
	$lastLogin = date("l, M j, Y, g:i a");
		...
		//Use the input username and password and check against 'admin' table
		$query = mysql_query('SELECT id, username, password, active, level_access FROM admin WHERE username = "'.secureInput($user).'" AND password = "'.secureInput($pass).'" AND level_access != 9') or die (mysql_error());
		
		if(mysql_num_rows($query) == 1)
		{
			$row = mysql_fetch_assoc($query);
			$update = mysql_query('UPDATE admin SET last_login = "'.$lastLogin.'" WHERE id = "'.$row['id'].'"');
			if ($row['active'] == 1 ) {
...
//----------Function for logging off admin----------
function logoff()
{
...
//----------checks the level access of admin----------
function checkLogin ( $levels )
{
		...
		if ( ! $_SESSION['logged_in'] ) {
			...
			$query = mysql_query('SELECT * FROM admin WHERE id = "'.mysql_real_escape_string($_COOKIE['cookie_id']).'"');
...
{
		$query = mysql_query("SELECT `level_access` FROM `admin` WHERE `id` = '" . 	mysql_real_escape_string ( $user_id ) . "'");
		if ( mysql_num_rows ( $query ) == 1 )
		{
...
function uniqueEmail($email)
{
	...
	$sql = "SELECT COUNT(*) as NUMBER FROM admin WHERE email = '" . $email ."' ";
	$res = mysql_query($sql);
...
//----------Function for checking existence of admin----------
...
function addUser($user,$pass,$email,$site_url)
{
	...
	$sql = "INSERT INTO admin (username,password,email,active,level_access,act_key,reg_date) VALUES ('".$user."','".$pass."','".$email."',1,1,'".$activation_key."','".$reg_date."')";
	...
	if($res){
		...
		<p>Dear ".$user.",</p>
		<p>These are the details of your admin account created at ".$site_url.". Please keep this email safe.</p>
		...
		<br/><br/>
		<p>Sincerely,</p>
		<p>Isvipi - Social Networking Script</p>
		</body>
		</html>
		";
...
function editUser($id,$email,$firstname,$lastname,$dialing_code,$phone,$city,$country)
{
  ...
  $dialing_code = secureInput($dialing_code);
	...
	if (!empty($email)){
			$sql = "UPDATE admin SET email = '" . $email . "', first_name = '" . $first_name . "', last_name = '" . $last_name . "', dialing_code = '" . $dialing_code . "', phone = '" . $phone . "', city = '" . $city . "', country = '" . $country . "' WHERE id = '" . $id . "'";		
			$res = mysql_query($sql) or die(mysql_error());
				if(!$res) return 4;
				return 99;
			} 
	if(empty($email)){
			$sql = "UPDATE admin SET first_name = '" . $first_name . "', last_name = '" . $last_name . "', dialing_code = '" . $dialing_code . "', phone = '" . $phone . "', city = '" . $city . "', country = '" . $country . "' WHERE id = '" . $id . "'";		
			$res = mysql_query($sql) or die(mysql_error());
				if(!$res) return 4;
				return 99;
			}
...
function updatePass($id,$opass,$pass)
{
	...
	$query = mysql_query('SELECT `password` FROM `admin` WHERE `id` = "'.$id.'"');
	$row = mysql_fetch_assoc($query);
...
function adminUpdatePass($uid,$pass)
{
	...
	$sql = "UPDATE admin SET password = '" . $new_password . "' WHERE id = '" . $uid . "'";
 	$res = mysql_query($sql);
	if($res) return 99;
		return 1;
}

//----------Function for deleting admin by admin----------
...
//----------Function for suspending admin by admin----------
function suspendUser($id)
{
	$sql = "SELECT id,active FROM admin WHERE id = '".$id."'"; 
	$res = mysql_query($sql);
	if ($res){
		$update = "UPDATE admin SET active = 2 WHERE id = '".$id."'";
...
//----------Function for reactivating admin by admin----------
function unsuspendUser($id)
{
	$sql = "SELECT id,active FROM admin WHERE id = '".$id."'"; 
	$res = mysql_query($sql);
	if ($res){
		$update = "UPDATE admin SET active = 1 WHERE id = '".$id."'";
		$result = mysql_query($update);
			if ($result)
				return 99;
					return 1;
...
function adminLogin($user,$pass)
{
	...
	$lastLogin = date("l, M j, Y, g:i a");
		...
		//Use the input username and password and check against 'admin' table
		$query = mysql_query('SELECT id, username, password, active, level_access FROM admin WHERE username = "'.secureInput($user).'" AND password = "'.secureInput($pass).'" AND level_access = 1') or die (mysql_error());
...
function pass_recovery($email,$site_url)
{
	...
	$sql = "SELECT id,username,password,email FROM admin WHERE email = '".$email."'";
	$res = mysql_query($sql) or die(mysql_error());
	$num = mysql_num_rows($res);
	...
	if($num == 1)
		...
		$update = mysql_query("UPDATE admin SET password='".$temp_pass."',temp_pass='".$temp_password."',temp_pass_active=1 WHERE email='".$email."'") or die(mysql_error());	
...
function confirm_pass($id,$new)
{
	...
	$query = mysql_query("SELECT password,temp_pass,temp_pass_active FROM admin WHERE id = '".$id."'");
	...
	if(mysql_num_rows($query)==1)
	{
		...
		{
			$update = mysql_query("UPDATE admin SET temp_pass_active=0 WHERE id = '".mysql_real_escape_string($_GET['id'])."'");
			if($update){
				return 99;
			} else return 1;
		}
...
function confirm_user_reg($activation_key)
{
	...
	$query = mysql_query("SELECT id,active,act_key FROM admin WHERE act_key = '".$activation_key."'");
	...
	if(mysql_num_rows($query)==1)
	{
		...
		if($row['active']==0)
		{
			$update = mysql_query("UPDATE admin SET active=1,act_key='' WHERE id = '".$id."'");
			if($update){
				return 99;
			} else return 1;
		}
...
//----------get countries via select option from admin----------
function get_select_countries($id)
{
	...
	$sql1 = "SELECT country FROM admin WHERE id='".$id."'";
...
function get_dialing_code($id)
{
	...
	$sql1 = "SELECT dialing_code FROM admin WHERE id='".$id."'";
	$res1 = mysql_query($sql1);
	$row1 = mysql_fetch_assoc($res1);
...
function uploadUserImage($path, $file, $tmpfile, $max, $id)
{
	...
	if (!empty($save_path)) {
		...
		$sql = "UPDATE admin SET thumb_path = '" . $save_thumb_path . "', img_path = '" . $save_path . "' WHERE id = '".$id."'";
		$res = mysql_query($sql);
...
function updateUserImage($path, $file, $tmpfile, $max, $id)
{
	...
	$sql = "SELECT thumb_path,img_path FROM admin WHERE id = '" . $id . "'";
	$res = mysql_query($sql);
	$row = mysql_fetch_assoc($res);
	$del = $row["thumb_path"];
	$delg = $row["img_path"];
...
function deleteImage($id)
{
	...
	$sql = "UPDATE admin SET thumb_path = '', img_path = '' WHERE id = '" . $id . "'";
	$res = mysql_query($sql);
...
{
	$sql = "SELECT thumb_path FROM admin WHERE id = '".$id."'";
	$res = mysql_query($sql);
	$row = mysql_fetch_assoc($res);
